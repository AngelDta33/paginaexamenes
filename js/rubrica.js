...
import {
  nuevoRubro, calificacionAlumno, sumaPorcentajes, validarRubros, calcularPromedio,
} from './gruposModel.js';

export function montarRubrica(contenedor, grupo) {
  clear(contenedor);
  ...
  function asegurarCalificacion(alumnoId) {
    if (!grupo.calificaciones[alumnoId]) grupo.calificaciones[alumnoId] = { valores: {}, extra: 0 };
  ...
  function pintarTabla() {
    ...
    const encabezado = el('tr', {}, [
      ...
      ...grupo.rubros.map((rubro) => el('th', { class: 'col-rubro' }, [
        el('input', {
          type: 'text', class: 'input-nombre-rubro', value: rubro.nombre, placeholder: 'Nombre del rubro',
          oninput: (e) => { rubro.nombre = e.target.value; guardarConDebounce(); },
        }),
        el('div', { class: 'fila-porcentaje-rubro' }, [
          ...
          el('button', {
            ...
            onclick: () => {
              ...
              for (const cal of Object.values(grupo.calificaciones)) delete cal.valores[rubro.id];
    ...
    const filas = alumnosActivos.map((alumno) => {
      ...
      function actualizarPromedio() {
        const p = calcularPromedio(grupo, alumno.id);
        celdaPromedio.textContent = p === null ? '—' : p.toFixed(2);
      }
      actualizarPromedio();

      const celdasRubro = grupo.rubros.map((rubro) => el('td', {}, [
        el('input', {
  ...
  contenedor.appendChild(el('div', { class: 'panel' }, [
    ...
    el('p', { class: 'etiqueta-chica' }, 'Calificaciones en escala 0–10. Puedes agregar o quitar rubros y cambiar los porcentajes cuando quieras — el promedio se recalcula solo.'),
    barraValidacion,
    el('div', { class: 'barra-nueva' }, [btnAgregarRubro]),
    contenedorTabla,
  ]));
